    ...
    while (k < n) {
        ...
        for (int i = 1; i <= n; i++)
        {
            if (arrnode[i].count == 0 && visit[i])
            {
                ...
                createListNode(n);
                //  DanhSachCount(n);
                ESP_topo = ESP_topo - radius - space;
                if (k == 1)
                {
                    drawText(ESP_topo + 60, 730, s2);
                }
                else
                {
                    hideNode(ESP_topo + 160, 605);
                    drawText(ESP_topo + 40, 610, s3);
                }
            }
        }
        if (kt == false && k < n)
...
void Queue_create()
{
    ...
    line(800, 650, ESP_topo + 50, 650);
    line(800, 730, ESP_topo + 50, 730);
}
void DiChuyenNode(int i)
{
    if (i % 2 == 1 && i != 1)
